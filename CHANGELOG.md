# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/), and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).


## [0.1.3] - 2026-03-03 

### Changed

- Loading docs dynamically with js

### Removed

- Site generation


## [0.1.2] - 2026-03-03 

### Changed

- Brought back marker type
- Better ui

### Fixed

- Parser
- Validating inputs


## [0.1.1] - 2026-03-02 

### Added

- Displaying lines
- Theme switch

### Changed

- Removed MarkerType


## [0.1.0] - 2026-03-02 

### Added

- Marker definitions
- Parsing files based on the markers
- Generating `tiny.docs.json` file
- Generating a searchable static site
- MIT License


[0.1.0]: https://github.com/KDesp73/tinydocs//releases/tag/v0.1.0
[0.1.1]: https://github.com/KDesp73/tinydocs//releases/tag/v0.1.1
[0.1.2]: https://github.com/KDesp73/tinydocs//releases/tag/v0.1.2
[0.1.3]: https://github.com/KDesp73/tinydocs//releases/tag/v0.1.3

